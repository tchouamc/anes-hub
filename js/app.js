...
const App = (() => {
  ...
  function renderDashboard() {
    ...
    // EPA hero ring
    const totalTarget = state.epas.reduce((s, e) => s + (e.target || 0), 0);
    const totalDone = state.epas.reduce((s, e) => s + Math.min(e.count || 0, e.target || 0), 0);
    ...
    // mini rings — show up to 3 EPAs as decorative progress
    const miniColors = ['#6B7FD8', '#0F6E56', '#B8860B', '#9B7FD8'];
    const miniHtml = state.epas.slice(0, 3).map((e, i) => {
      const p = e.target ? Math.min(1, e.count / e.target) : 0;
      const circ = 69.1;
      return `<div class="mini-ring-row">
        <svg class="mini-ring-svg" viewBox="0 0 28 28">
          ...
          <circle cx="14" cy="14" r="11" fill="none" stroke="${miniColors[i % miniColors.length]}" stroke-width="4" stroke-linecap="round" stroke-dasharray="${circ}" stroke-dashoffset="${circ - circ * p}" transform="rotate(-90 14 14)"/>
        </svg>
        <span class="mini-ring-label">${esc(e.name)}</span>
      </div>`;
    }).join('');
    document.getElementById('mini-rings').innerHTML = miniHtml || '<div class="empty-state-mini">Add EPAs in Requirements & goals.</div>';
  ...
  function activityForDate(dStr) {
    const acts = [];
    if (state.journalEntries.some(j => j.date === dStr)) acts.push('journal');
    if (state.cases.some(c => normalizeDate(c.date) === dStr)) acts.push('case');
    if (state.habitLog.some(h => h.date === dStr)) acts.push('habit');
    return acts;
  }
  ...
  function buildCalGrid(year, month, mini) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dotColors = { journal: '#B8860B', case: '#0F6E56', habit: '#6B7FD8' };
      const dots = acts.map(a => `<div class="cal-dot" style="background:${dotColors[a]}"></div>`).join('');
  ...
  function renderCalendar() {
    const label = new Date(state.calYear, state.calMonth, 1).toLocaleString('default', { month: 'long', year: 'numeric' });
    document.getElementById('cal-month-label').textContent = label;
    document.getElementById('cal-grid-full').innerHTML = buildCalGrid(state.calYear, state.calMonth, false);
    renderUpcoming('cal-upcoming-mini-placeholder'); // no-op guard
    renderCalUpcoming();
  }
  ...
  function renderCalUpcoming() {
    ...
    if (!el) return;
    if (!items.length) { el.innerHTML = '<div class="empty-state-mini">Nothing due.</div>'; return; }
    el.innerHTML = items.slice(0, 5).map(item => {
      const badge = guessUrgency(item.due);
      return `<div class="upcoming-mini-item">
        <div class="up-check-mini" onclick="App.completeRequirement('${item._id}')"></div>
  ...
  function selectCalDay(dStr) {
    ...
    if (journal) html += `<div class="detail-item"><div class="detail-dot" style="background:#B8860B"></div><div><div class="detail-text">Journal entry</div><div class="detail-sub">${esc(journal.text.slice(0,60))}…</div></div></div>`;
    const dayCases = state.cases.filter(c => normalizeDate(c.date) === dStr);
    dayCases.forEach(c => {
      html += `<div class="detail-item"><div class="detail-dot" style="background:#0F6E56"></div><div><div class="detail-text">${esc(c.title)}</div><div class="detail-sub">${esc(c.rotation || '')}</div></div></div>`;
    });
    const habit = state.habitLog.find(h => h.date === dStr);
    if (habit) html += `<div class="detail-item"><div class="detail-dot" style="background:#6B7FD8"></div><div><div class="detail-text">Studied</div><div class="detail-sub">${esc((habit.topics||[]).join(', ') || 'No topics logged')}</div></div></div>`;
    if (!html) html = '<div class="empty-state-mini">Nothing logged this day.</div>';
    body.innerHTML = html;
  }
  ...
  function openEPAForm() { document.getElementById('epa-form').classList.toggle('hidden'); }
  ...
  async function addEPA() {
    const name = document.getElementById('epa-name').value.trim();
    const target = parseInt(document.getElementById('epa-target').value) || 1;
    if (!name) return;
    const epa = { name, target, count: 0, done: false };
    state.epas.push(epa);
    document.getElementById('epa-name').value = '';
    document.getElementById('epa-target').value = '';
    document.getElementById('epa-form').classList.add('hidden');
    renderAll();
    saveCache();
    try {
      epa._id = await createRequirement({ text: name, cat: 'EPA', due: '', done: false, notes: JSON.stringify({ target, count: 0 }) });
      saveCache();
      toast('EPA added and synced to Notion');
    } catch { toast('Saved locally — sync to push to Notion'); }
  }

  async function logEPAInstance(idx) {
    const epa = state.epas[idx];
    if (!epa) return;
    epa.count = (epa.count || 0) + 1;
    if (epa.count >= epa.target) epa.done = true;
    renderAll();
    saveCache();
    if (epa._id) {
      try {
        await updateRequirement(epa._id, { done: epa.done, notes: JSON.stringify({ target: epa.target, count: epa.count }) });
        toast(epa.done ? `${epa.name} complete!` : 'Logged');
      } catch { toast('Saved locally — sync to push to Notion'); }
    }
  }
  ...
  function renderRequirementsGoals() {
    ...
    if (!state.epas.length) {
      epaList.innerHTML = '<div class="empty-state">No EPAs yet. Add one above with its target count.</div>';
    } else {
      epaList.innerHTML = state.epas.map((e, i) => {
        const pct = e.target ? Math.min(100, Math.round((e.count / e.target) * 100)) : 0;
        if (e.done) {
          return `<div class="epa-counter-row complete">
            <div class="epa-status-icon complete"><svg viewBox="0 0 24 24" fill="none" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></div>
            <div class="epa-row-info">
              <div class="epa-row-name complete">${esc(e.name)}</div>
              <div class="epa-mini-progress"><div class="epa-mini-bar"><div class="epa-mini-bar-fill complete" style="width:100%;"></div></div><span class="epa-count-text complete">${e.count} / ${e.target}</span></div>
            </div>
            <div class="epa-done-badge"><svg viewBox="0 0 24 24" fill="none" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>Done</div>
          </div>`;
        }
        return `<div class="epa-counter-row">
          <div class="epa-status-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg></div>
          <div class="epa-row-info">
            <div class="epa-row-name">${esc(e.name)}</div>
            <div class="epa-mini-progress"><div class="epa-mini-bar"><div class="epa-mini-bar-fill" style="width:${pct}%;"></div></div><span class="epa-count-text">${e.count} / ${e.target}</span></div>
          </div>
          <div class="epa-increment-btn" onclick="App.logEPAInstance(${i})"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14"/></svg>Log one</div>
        </div>`;
      }).join('');
    }
    ...
    blockEl.innerHTML = blockGoals.length ? blockGoals.map(g => goalRowHTML(g, 'mandatory')).join('') : '<div class="empty-state">No block requirements yet.</div>';

    // Personal goals
    const personalGoals = state.requirements.filter(r => r.cat === 'Personal');
    const persEl = document.getElementById('personal-goal-list');
    persEl.innerHTML = personalGoals.length ? personalGoals.map(g => goalRowHTML(g, 'personal')).join('') : '<div class="empty-state">No personal goals yet.</div>';
  }

  function goalRowHTML(g, kind) {
    const tagLabel = kind === 'mandatory' ? 'Block goal' : 'Personal';
    return `<div class="goal-row ${kind}">
      <div class="goal-check ${g.done ? 'done' : ''}" onclick="App.toggleGoal('${g._id}')"></div>
      <div style="flex:1;"><div class="goal-text ${g.done ? 'done' : ''}">${esc(g.text)}</div></div>
      <span class="goal-tag ${kind}">${tagLabel}</span>
      ${g.due ? `<span class="goal-due ${guessUrgency(g.due).cls === 'urgent' ? 'urgent' : 'normal'}">${esc(g.due)}</span>` : ''}
    </div>`;
  }
