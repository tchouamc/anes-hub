...
const App = (() => {
  ...
  // ════════ CASE LOG SUBTABS ════════
  let _currentCaseSubtab = 'cases';
  let _statsPeriod = 'all';
  let _selectedProcTypes = [];

  function switchCaseSubtab(tab) {
    _currentCaseSubtab = tab;
    ['cases','procedures','stats'].forEach(t => {
      document.getElementById('casesub-' + t).classList.toggle('hidden', t !== tab);
      document.getElementById('subtab-' + t).classList.toggle('active', t === tab);
    });
    if (tab === 'procedures') renderProcLog();
    if (tab === 'stats') renderProcStats();
  }

  // ── Procedure overlay ──
  function openProcOverlay() {
    _selectedProcTypes = [];
    document.getElementById('proc-date').value = new Date().toLocaleDateString('default', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' });
    document.getElementById('proc-supervisor').value = '';
    document.getElementById('proc-description').value = '';
    document.getElementById('proc-improvement').value = '';
    document.getElementById('proc-custom-type').value = '';
    document.querySelectorAll('.proc-type-chip').forEach(c => c.classList.remove('selected'));
    document.getElementById('proc-overlay').classList.remove('hidden');
  }

  function closeProcOverlay() {
    document.getElementById('proc-overlay').classList.add('hidden');
  }

  function addCustomProcType() {
    const input = document.getElementById('proc-custom-type');
    const val = input.value.trim();
    if (!val) return;
    if (!_selectedProcTypes.includes(val)) _selectedProcTypes.push(val);
    // Add chip to picker
    const picker = document.getElementById('proc-type-picker');
    const existing = picker.querySelector(`[data-proc="${CSS.escape(val)}"]`);
    if (!existing) {
      const chip = document.createElement('div');
      chip.className = 'proc-type-chip selected';
      chip.dataset.proc = val;
      chip.textContent = val;
      chip.onclick = () => {
        chip.classList.toggle('selected');
        if (chip.classList.contains('selected')) _selectedProcTypes.push(val);
        else _selectedProcTypes = _selectedProcTypes.filter(p => p !== val);
      };
  ...
  // Bind proc type chip clicks on DOMContentLoaded (in existing DOMContentLoaded block)
  function bindProcTypeChips() {
    document.querySelectorAll('.proc-type-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        chip.classList.toggle('selected');
        const proc = chip.dataset.proc;
        if (chip.classList.contains('selected')) _selectedProcTypes.push(proc);
        else _selectedProcTypes = _selectedProcTypes.filter(p => p !== proc);
      });
    });
  }

  function saveProcedure() {
    if (!_selectedProcTypes.length) return toast('Select at least one procedure type.');
    if (!state.procedures) state.procedures = [];
    const proc = {
      id: Date.now().toString(),
      date: document.getElementById('proc-date').value.trim(),
      types: [..._selectedProcTypes],
      supervisor: document.getElementById('proc-supervisor').value.trim(),
      description: document.getElementById('proc-description').value.trim(),
      improvement: document.getElementById('proc-improvement').value.trim(),
    };
    ...
    renderProcLog();
    renderProcStats();
    toast('Procedure logged');
  }

  function deleteProcedure(id) {
    if (!confirm('Delete this procedure log?')) return;
  ...
  function renderProcLog() {
    ...
    const sub = document.getElementById('proc-log-sub');
    if (sub) sub.textContent = procs.length + ' procedure' + (procs.length !== 1 ? 's' : '') + ' logged';
    const list = document.getElementById('proc-log-list');
    if (!list) return;
    if (!procs.length) { list.innerHTML = '<div class="empty-state">No procedures logged yet. Tap "Log procedure" to start.</div>'; return; }
    list.innerHTML = procs.map(p => {
      const types = (p.types || []).map(t => `<span class="proc-type-badge">${esc(t)}</span>`).join(' ');
      return `<div class="proc-log-row" id="proc-row-${p.id}" onclick="App.toggleProcRow('${p.id}')">
        <div class="proc-row-head">
          <div style="display:flex; gap:6px; flex-wrap:wrap; flex:1;">${types}</div>
          ${p.supervisor ? `<span class="proc-row-supervisor">Dr. ${esc(p.supervisor.replace(/^Dr\.?\s*/i, ''))}</span>` : ''}
          <span class="proc-row-date">${esc(p.date)}</span>
        </div>
        <div class="proc-row-body">
          ${p.description ? `<div class="proc-detail-label">Description</div><div class="proc-detail-text">${esc(p.description)}</div>` : ''}
          ${p.improvement ? `<div class="proc-detail-label" style="color:var(--amber);">Areas for improvement</div><div class="proc-improvement-text">${esc(p.improvement)}</div>` : ''}
          <div class="proc-row-actions">
            <div class="case-action-btn delete" onclick="event.stopPropagation(); App.deleteProcedure('${p.id}')">
  ...
  function renderProcStats() {
    const grid = document.getElementById('proc-stats-grid');
    if (!grid) return;
    const procs = state.procedures || [];
    const now = new Date();
    ...
    const filtered = procs.filter(p => {
      if (_statsPeriod === 'all') return true;
      const d = new Date(p.date);
      if (isNaN(d)) return true;
      if (_statsPeriod === 'month') return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
      if (_statsPeriod === 'year') return d.getFullYear() === now.getFullYear();
      if (_statsPeriod === 'block') {
        // Block = current 4-week window from residency start
        const start = new Date(state.startDate || '2026-07-01');
        const daysSinceStart = Math.floor((now - start) / 86400000);
        const blockNum = Math.floor(daysSinceStart / 28);
        const blockStart = new Date(start.getTime() + blockNum * 28 * 86400000);
        const blockEnd = new Date(blockStart.getTime() + 28 * 86400000);
    ...
    filtered.forEach(p => {
      (p.types || []).forEach(t => {
        ...
        if (!lastSeen[t] || p.date > lastSeen[t]) lastSeen[t] = p.date;
      });
    });

    const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]);
    const periodLabel = { all: 'all time', month: 'this month', block: 'this block', year: 'this year' }[_statsPeriod];

    if (!sorted.length) {
      grid.innerHTML = `<div class="proc-stats-empty">No procedures logged for ${periodLabel}.</div>`;
      return;
    }

    grid.innerHTML = `<div class="proc-stats-grid">${sorted.map(([type, count]) => `
      <div class="proc-stat-card">
        <div class="proc-stat-type">${esc(type)}</div>
        <div class="proc-stat-count">${count}</div>
        <div class="proc-stat-label">${count === 1 ? 'procedure' : 'procedures'} — ${periodLabel}</div>
        ${lastSeen[type] ? `<div class="proc-stat-last">Last: ${esc(lastSeen[type])}</div>` : ''}
      </div>`).join('')}</div>`;
  }
  ...
  return {
    ...
    switchCaseSubtab, openProcOverlay, closeProcOverlay,
    saveProcedure, deleteProcedure, toggleProcRow,
    addCustomProcType, setStatsPeriod,
